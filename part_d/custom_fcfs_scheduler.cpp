...
class FCFS : public IScheduler, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IScheduler, FCFS, "FCFS", "Pure FCFS DRAM Scheduler.")
  private:
    IDRAM* m_dram;
  ...
  public:
    ...
    ReqBuffer::iterator compare(ReqBuffer::iterator req1, ReqBuffer::iterator req2) override {
      // Pure FCFS: ignore row hits/readiness, strictly pick the earliest arrival time
      if (req1->arrive <= req2->arrive) {
        return req1;
      } else {
        return req2;
